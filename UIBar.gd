...
func _ready():
  ...
  new_label("+10", load("res://NumberFont.tres"), Vector2(-4, -2), discard)
  new_label("Depleted", load("res://LabelFont.tres"), Vector2(-8, 10), discard, 0.1666)
  ...
  for i in range(1, player.deckSize+1):
    ...
    new_label("1", load("res://NumberFont.tres"), Vector2(5, -8), slot)
    var s = new_sprite(load("res://assets/pom.png"), 0.3333333, Vector2(5,6), slot)  #pom icon
    s.visible = false
    new_label(str(i), load("res://LabelFont.tres"), Vector2(-1, 10), slot, 0.1666)
    
  available = new_sprite( load("res://assets/boonslot_filled.png"), 3, Vector2(boonX + 6 * boonXinc, boonY))
  new_label("+10", load("res://NumberFont.tres"), Vector2(-4, -2), available)
  new_label("Available", load("res://LabelFont.tres"), Vector2(-8, 10), available, 0.1666)
...
func _draw():
  ...
  moreText.text = str(player.money) + " obols"
  
  var reload = "\n(R to draw)" if player.is_hand_done() and player.boons.size() > 4 else ""
  available.get_child(1).text = "Available" + reload
