...
onready var audioPlayer = $"../EnemySfx"
...
func move(nx, ny):
  audioPlayer.play_sample("defaultMove")
  .move(nx, ny)
...
onready var Atk = load("res://particles/MegFloorAtkSprite.tscn")
...
func meg_ai():
  ...
  match state.currentState:
    ...
    _:
      state.currentState = "attack"    #initial phase
